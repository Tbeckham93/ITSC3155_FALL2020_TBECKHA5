...
# notes_list = { 1:{'title':'First note', 'text':'This is my first note', 'date':'10-1-2020'} ,
...
def get_note(note_id):
    ...
    if session.get('user'):
        ...
        form = CommentForm()
        return render_template('note.html', user=session['user'], note=my_note, form=form)
    else:
        return redirect(url_for('login'))
...
def new_notes():
    if session.get('user'):
        if request.method == 'POST':
            ...
            newEntry = Note(title, text, today, user_id=session['user'])
